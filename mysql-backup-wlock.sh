#!/bin/bash
#
# If the database to be backup is ALL, then this script will backup MySQLs
# with parameter --lock-all-tables.
...
if [ "$DBNAMES" = "all" ]; then
	mysql --user=$USERNAME --password=$PASSWORD --host=$DBHOST --port=$DBPORT --batch --skip-column-names -e "show databases"
...
tar zcf $BACKUPDIR/`basename $DBCONF`.tar.gz -C /home/mysync `basename $DBCONF`
